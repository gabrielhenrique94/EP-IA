...
public class LVQ {
	...
	public void treinamentoLVQ(boolean temValidacao) {
		...
		while (this.epocas <= this.max_epocas || this.erroAtual < this.erroMax) {	
			...
			if (temValidacao){
				...
				System.out.println("Erro atual �: " + erroAtual);
			}
			this.epocas++;
		}
	...
	public void teste(){
		...
		try {
			...
			i.write("Parou na �poca" + this.epocas + "\n");
			i.write("M�ximo de �pocas passada por par�metro" + max_epocas);
			i.close();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		if(erroAtual < erroMax){
			System.out.println("Ap�s treinamento, o erro final � menor que erro esperado no momento de validar: " + this.erroAtual );
			System.out.println("O n�mero total de �pocas foi: " + this.epocas);
		} 
		System.out.println("Ap�s o treinamento, o erro final � maior que o esperado, mas j� atingimos o m�ximo de �pocas. Erro atual: " + this.erroAtual);
	}
}